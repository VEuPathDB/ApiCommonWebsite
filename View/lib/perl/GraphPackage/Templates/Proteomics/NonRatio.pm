...
sub getSpecs {
  return [ {abbrev => "Apico",
            ...
            query => "SELECT ga.source_id, CASE WHEN (nafe.value = 0 ) THEN 1e-9 ELSE nafe.value END as value FROM webready.GeneAttributes_p ga, results.nafeatureexpression nafe, study.protocolappnode pan, study.nodenodeset nns, study.nodeset ns WHERE nafe.na_feature_id = ga.na_feature_id AND pan.protocol_app_node_id = nns.protocol_app_node_id AND nafe.protocol_app_node_id = nns.protocol_app_node_id AND nns.node_set_id = ns.node_set_id AND ns.NAME = 'Apicoplast and ER Proteomes' AND pan.NAME LIKE 'Apicoplast%'",
           },
           {abbrev => "ER",
            name => "ER Abundance",
            query => "SELECT ga.source_id, CASE WHEN (nafe.value = 0 ) THEN 1e-9 ELSE nafe.value END as value FROM webready.GeneAttributes_p ga, results.nafeatureexpression nafe, study.protocolappnode pan, study.nodenodeset nns, study.nodeset ns WHERE nafe.na_feature_id = ga.na_feature_id AND pan.protocol_app_node_id = nns.protocol_app_node_id AND nafe.protocol_app_node_id = nns.protocol_app_node_id AND nns.node_set_id = ns.node_set_id AND ns.NAME = 'Apicoplast and ER Proteomes' AND pan.NAME LIKE 'ER%'",
