...
sub getPhenotypeSpecs {
  return [ {abbrev => "MIS",
            ...
            query => "select ga.source_id, r.score as value from apidb.phenotypescore r, webready.GeneAttributes_p ga where ga.na_feature_id = r.na_feature_id and r.score_type = 'mutagenesis index score'",
...
gp = gp + annotate(\"text\", x = 5000, y = 0.9, label = \"Dispensable\", colour = '#d3883f');"
           ...
           {abbrev => "MFS",
            ...
            query => "select ga.source_id, r.score as value from apidb.phenotypescore r, webready.GeneAttributes_p ga where ga.na_feature_id = r.na_feature_id and r.score_type = 'mutant fitness score'"
           },
      ];
}
...
sub getPhenotypeSpecs {
  return [ {abbrev => "OIS",
            ...
            query => "select ga.source_id, r.score as value from apidb.phenotypescore r, webready.GeneAttributes_p ga where ga.na_feature_id = r.na_feature_id and r.score_type = 'Occupancy index score'"
           },
           {abbrev => "HMS",
            name => "Hybrid model score",
            query => "select ga.source_id, r.score as value from apidb.phenotypescore r, webready.GeneAttributes_p ga where ga.na_feature_id = r.na_feature_id and r.score_type = 'Hybrid model score'"
           },
      ];
}
...
sub getPhenotypeSpecs {
  return [ {abbrev => "MIS plus",
            name => "MIS+ score",
            query => "select ga.source_id, r.score as value from apidb.phenotypescore r, webready.GeneAttributes_p ga where ga.na_feature_id = r.na_feature_id and r.score_type = 'MIS plus'"
           },
           {abbrev => "MIS",
            name => "MIS score",
            query => "select ga.source_id, r.score as value from apidb.phenotypescore r, webready.GeneAttributes_p ga where ga.na_feature_id = r.na_feature_id and r.score_type = 'MIS'"
           },
           {abbrev => "MFS",
            name => "MFS score",
            query => "select ga.source_id, r.score as value from apidb.phenotypescore r, webready.GeneAttributes_p ga where ga.na_feature_id = r.na_feature_id and r.score_type = 'MFS'"
           },
      ];
}
...
sub getPhenotypeSpecs {
  return [ {abbrev => "rel_growth_rate",
            ...
            query => "select ga.source_id, r.relative_growth_rate as value from APIDB.PHENOTYPEGROWTHRATE r, webready.GeneAttributes_p ga where r.na_feature_id = ga.na_feature_id"
           },
      ];
}
...
sub getPhenotypeSpecs {
  return [ {abbrev => "phenotype_score",
            ...
            query => "select ga.source_id, r.mean_phenotype as value from APIDB.CRISPRPHENOTYPE r, webready.GeneAttributes_p ga where ga.na_feature_id = r.na_feature_id", 
