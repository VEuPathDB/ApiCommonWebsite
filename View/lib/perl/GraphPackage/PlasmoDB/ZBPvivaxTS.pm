package ApiCommonWebsite::View::GraphPackage::PlasmoDB::ZBPvivaxTS;
...
sub setBioRep03ExpressionQuery    { $_[0]->{'BioRep03ExpressionQuery'   } = $_[1]; $_[0] }
sub getBioRep03ExpressionQuery    { $_[0]->{'BioRep03ExpressionQuery'   } }
...
sub setBioRep03PercentileQuery    { $_[0]->{'BioRep03PercentileQuery'   } = $_[1]; $_[0] }
sub getBioRep03PercentileQuery    { $_[0]->{'BioRep03PercentileQuery'   } }
...
sub makeR {
   ...
   $names = '"9", "13","17","20","23","29","35","40","43"';
   ...
   my $biorep03Exp_f = eval { $Self->getBioRep03ExpressionQuery()->makeTabFile($_qh, $_dict) }; $@ && push(@errors, $@);
   ...
   my $biorep03Per_f = eval { $Self->getBioRep03PercentileQuery()->makeTabFile($_qh, $_dict) }; $@ && push(@errors, $@);
...
color.rep3      <- rgb(153, 153,   0, max=255);

colors.track1   <- c(color.rep1);
colors.track2   <- c(color.rep2);
colors.track3   <- c(color.rep3);

pct.colors      <- c(color.rep1, color.rep2, color.rep3);
...
data.track3.table    <- read.table("$biorep03Exp_f", header=T);

data.track1.rat <- c(data.track1.table\$VALUE);
data.track2.rat <- c(data.track2.table\$VALUE);
data.track3.rat <- c(data.track3.table\$VALUE);

data.percents1.table    <- read.table("$biorep01Per_f", header=T);
data.percents2.table    <- read.table("$biorep02Per_f", header=T);
data.percents3.table    <- read.table("$biorep03Per_f", header=T);

data.percents1 <- data.percents1.table\$VALUE;
data.percents2 <- data.percents2.table\$VALUE;
data.percents3 <- data.percents2.table\$VALUE;

pct.all <- rbind(data.percents1, data.percents2, data.percents3);

# ------------------------ Merge Data and Colors -------------------------

ylim.rat <- c(max(-10, min(-2,min(data.track1.rat, data.track2.rat, data.track3.rat))),
                       min( 10, max( 2,max(data.track1.rat, data.track2.rat, data.track3.rat)))
              );
...
if ($isVis_b{LEGEND} == 1) {
  ...
  legend(0.5, 0.5,
         c("Patient 1", "Patient 2", "Patient 3"),
         xjust = 0.5,
         yjust = 0.5,
         cex   = 0.90,
         bty   = "n",
         lty   = "solid",
         col   = c(color.rep1, color.rep2, color.rep3),
         pt.bg = c(color.rep1, color.rep2, color.rep3),
         pch   = 22,
         horiz = T
        );
}
...
if ($isVis_b{rat} == 1) {
  ...
  lines (seq(1,9),
         data.track2.rat[seq(1,9)],
         type    = "c",
         col     = "gray80"
        );
  lines (seq(1,9),
         data.track3.rat[seq(1,9)],
         type    = "c",
         col     = "gray80"
        );
  ...
  plasmodb.title("Differentiation Time Series");
}
