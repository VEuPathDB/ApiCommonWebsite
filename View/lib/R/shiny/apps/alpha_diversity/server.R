...
#      "Characteristics.tab",
  ...
  for (i in 1:length(corrected_columns)) {
  	hash_sample_names[[columns[i]]] <- corrected_columns[i]
  }
  ...
  physeq <- phyloseq(OTU, TAX, SAMPLE)
	...
	output$abundanceChart <- renderPlot({
	    if(identical(input$category, "All Samples")){
	      chart <- plot_richness(physeq, measures = input$measureCheckBox)+theme(
	        panel.grid.major.x = element_blank()
	      )+geom_point(size = 4, alpha= 0.5)
	    }else{
	      chart <- plot_richness(physeq, x=hash_sample_names[[input$category]], measures = input$measureCheckBox)+theme(
	        panel.grid.major.x = element_blank()
	      )+geom_point(size = 4, alpha= 0.5)
	    }
	...
	output$hover_info <- renderUI({
	    ...
	    if(identical(input$category, "All Samples")){
	      ...
	      if(nrow(near_points) > 0){
	        category_value <- sample_data(physeq)[,hash_sample_names[[input$category]]]
