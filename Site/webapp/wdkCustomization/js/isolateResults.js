...
function goToIsolate(ele,type,source_id,start,end) {
  ...
  if(cbs.length < 2) {
    alert("Please select at least two isolates to run Clustal Omega");
    return false;
  }
  var url = "/cgi-bin/isolateAlignment?project_id=" + wdk.modelName() + ";type=" + type + ";sid=" + source_id + ";start=" + start + ";end=" + end + ";isolate_ids=";
  cbs.each(function(){
    url += $(this).val() + ",";
  });
  //alert(url);
  /* code if we want to popup a new window */
  var w = open ('', 'clustalResult', 'width=800,height=500,titlebar=1,menubar=1,resizable=1,scrollbars=1,toolbar=1');
  w.document.open();
  w.location.href=url;
