...
export default function GalaxyTerms(props) {
  let { user, goToGalaxy, setShowPagePreference, showLoginForm, showPagePreference } = props;
  return (
      ...
      <div id="eupathdb-GalaxyTerms-submit">
        {user.isGuest && (
          <p style={{ fontSize: '1.2em' }}>
            Note: You must first <a href="#login" onClick={e => { e.preventDefault(); showLoginForm() }}>login</a> to
            use EuPathDB Galaxy.</p>)}
        <input
          name="galaxySubmit"
          disabled={user.isGuest}
          value="Go to Galaxy"
          title="Click to go to galaxy"
          type="submit"
          onClick={goToGalaxy}
        />
        <p style={{ color: user.isGuest ? 'darkgray' : '' }}>
          <label>
            <input
              type="checkbox"
              checked={!showPagePreference}
              disabled={user.isGuest}
...
GalaxyTerms.propTypes = {
  ...
  showPagePreference: PropTypes.bool.isRequired,
  goToGalaxy: PropTypes.func.isRequired,
  setShowPagePreference: PropTypes.func.isRequired,
