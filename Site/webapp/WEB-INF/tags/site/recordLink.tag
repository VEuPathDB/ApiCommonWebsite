...
<jsp:root version="2.0"
  ...
  <c:set var="basket">
                          ...
                          <c:set var="basketId" value="basket${fn:replace(primaryKeyAttributeValue.value,'.','_')}-trView" />
                          <c:choose>
                            <c:when test="${!wdkUser.guest}">    <!-- REGISTERED USER -->
                              ...
                              <c:set var="basketClick" value="wdk.basket.updateBasket(this,'single', '${primaryKeyAttributeValue.value}', '${modelName}', '${recordName}')" />
                            </c:when>
