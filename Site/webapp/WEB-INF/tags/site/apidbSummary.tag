...
<c:set value="${wdk_paging_end - wdk_paging_start + 1}" var="pageSize"/>
             ...
             <c:forEach items="${wdkAnswer.resultSizesByProject}" var="rSBP">
                <c:choose>
                  <c:when test="${rSBP.key == 'cryptodb'}">&nbsp;&nbsp; 
<a href="showSummary.do?wdk_history_id=${historyId}&pager.offset=0">
...
<a href="showSummary.do?wdk_history_id=${historyId}&pager.offset=${Poffset}">
...
<a href="showSummary.do?wdk_history_id=${historyId}&pager.offset=${Toffset}">
ToxoDB: ${rSBP.value}</a>
                  </c:when>
                </c:choose>
             </c:forEach>
