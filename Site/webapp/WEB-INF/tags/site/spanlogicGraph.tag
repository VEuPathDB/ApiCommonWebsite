...
<div id="group_${groupName}" class="regionParams">
  <div class="regionHeader">Specify a <span class="region">region</span> relative to each of the ${step.resultSize} ${pluralType} in Step <span class="${stepType}_num"></span></div>
  ...
  <fieldset id="set_${groupName}Fields">
    <ul class="horizontal">
      ...
      <li>
        ...
        <input type="text" class="regionText" name="upstream_region_${groupName}" />&nbsp;bp
      </li>
      <li>
        <input type="radio" name="region_${groupName}" value="downstream" onclick="${regionOnClick}">Downstream</input>
        <input type="text" class="regionText" name="downstream_region_${groupName}" />&nbsp;bp
      </li>
