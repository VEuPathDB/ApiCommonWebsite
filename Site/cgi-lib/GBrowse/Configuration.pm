...
sub getSyntenySubtracks {
    my ($self) = @_;
    my $dbh = $self->dbh();
    my $sh = $dbh->prepare("select organism, public_abbrev,  phylum, kingdom, genus, species, class  from webready.OrganismSelectTaxonRank_p order by kingdom, class, phylum, genus, species, organism");
    $sh->execute();
    my @rv;
