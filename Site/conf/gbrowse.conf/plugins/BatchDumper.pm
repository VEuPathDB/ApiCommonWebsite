...
sub dump {
  ...
  unless (@segments) {
   ...
   # A quick hack to dump protein sequences from GUS
   # Added by Haiming Wang Dec 02, 2005 
   if ($config->{'type'} eq 'protein') {
	  foreach my $segment (@segments) {

		my ($nuc, $pro, $seq) = @{$segment->protein($segment->feature_id)};
		$seq =~ s/(\S{60})/$1\n/g;
		$pro =~ s/\.(.*?)$//i;	# trim off the protein_id's version number
		print ">$pro - $nuc\n$seq\n";
	}
		return;
   }
...
sub configure_form {
  ...
  # Allow for selecting sequence type - nucleotide and protein
  # Added by Haiming Wang Dec 02, 2005
  push @choices, TR({-class => 'searchtitle'},
			th({-align=>'RIGHT',-width=>'25%'},"type",
			   td(radio_group('-name'   => $self->config_name('type'),
					  '-values' => [qw(nucleotide protein)],
					  '-default'=> $current_config->{'type'},
					  -labels   => {'nuc' => 'nucleotide',
							'pro' => 'protein',
						       },
					  '-override' => 1))));
