...
public class ApiSiteSetup {
   ...
   * operator (e.g. intersect, union) is revised.  It will set the value to the
   * default for the new operator.
   */
  private static final EventListener TX_BOOLEAN_REVISE_LISTENER = new EventListener() {
    ...
    public void eventTriggered(Event event) throws Exception {
      ...
      if (!revisedStep.isBoolean() ||
          !isTranscriptRecordClass(revisedStep.getRecordClass())) {
      ...
      if (geneBooleanFilter == null) {
        throw new WdkModelException(
            "Found transcript boolean step " + revisedStep.getStepId() +
                " without GeneBooleanFilter.");
      }

      if (!geneBooleanFilter.isSetToDefaultValue(revisedStep)) {
        JSONObject newValue = GeneBooleanFilter.getDefaultValue(newOperator);
        LOG.info(
            "Resetting gene boolean filter on step " + revisedStep.getStepId() +
                " to default value: " + newValue.toString(2));
        revisedStep.addFilterOption(
            GeneBooleanFilter.GENE_BOOLEAN_FILTER_ARRAY_KEY, newValue);
        revisedStep.saveParamFilters();
      }
    }

    private String getOperator(Step revisedStep) throws WdkModelException {
      String operator = revisedStep.getParamValues()
          .get(BooleanQuery.OPERATOR_PARAM);

      if (operator == null) {
        throw new WdkModelException(
            "Found transcript boolean step " + revisedStep.getStepId() +
                " without " + BooleanQuery.OPERATOR_PARAM + " parameter.");
      }
