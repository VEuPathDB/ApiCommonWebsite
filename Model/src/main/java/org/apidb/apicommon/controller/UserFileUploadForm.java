...
public class UserFileUploadForm extends ActionForm {
  ...
  private HashMap<Integer, FormFile> formFiles;
  private HashMap<Integer, String> formNotes;
  ...
  public ActionErrors validate(ActionMapping mapping,
    ...
    if (getTitle() == null || getTitle().trim().length() == 0) {
      errors.add(ActionErrors.GLOBAL_MESSAGE,
          new ActionMessage("mapped.properties", "no title",
              "please add a title"));
    }
    ...
    if (getTitle().trim().length() > 4000) {
      errors.add(ActionErrors.GLOBAL_MESSAGE,
          new ActionMessage("mapped.properties",
              "title is too long (" + getTitle().trim().length() +
                  " characters)",
