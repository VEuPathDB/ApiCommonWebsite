  ...
  public ActionErrors validate(ActionMapping mapping, HttpServletRequest request) {
    ...
    if (maxLengthExceeded != null && maxLengthExceeded.booleanValue()) {
      errors.add(ActionErrors.GLOBAL_MESSAGE,
          new ActionMessage("mapped.properties", "file upload is larger than the allowed " + maxFileSize,
    ...
    while (it.hasNext()) {
      Integer i = it.next();
      if (formFiles.get(i).getFileName().trim().length() == 0 && formNotes.get(i).trim().length() == 0) {
        it.remove();
        formNotes.remove(i);
      }
    }
    ...
    if (getTitle().trim().length() > 4000) {
      errors.add(ActionErrors.GLOBAL_MESSAGE,
          new ActionMessage("mapped.properties",
              "title is too long (" + getTitle().trim().length() + " characters)",
    ...
    for (Integer i : formNotes.keySet()) {
      ...
      if (formNotes.get(i) != null && formNotes.get(i).trim().length() > 4000) {
        errors.add(ActionErrors.GLOBAL_MESSAGE,
            new ActionMessage("mapped.properties",
                "description is too long (" + formNotes.get(i).trim().length() + " characters)",
...
/**
 * validation: Notes <= 4000 chars filename <= 255 chars
 **/
