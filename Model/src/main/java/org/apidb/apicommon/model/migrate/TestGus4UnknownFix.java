...
public class TestGus4UnknownFix {
  ...
  public void testFilterFix() throws Exception {
    try (var model = WdkModel.construct(PROJECT_ID,
      GusHome.getGusHome())) {
      for (int i = 0; i < TEST_CASES.length; i++) {
        var step = getStep(i);
        var plugin = new Gus4StepTableMigrator();
        // create temporary empty file
        var emptyQuestionMappingFile = Files.createTempFile(
          TestGus4UnknownFix.class.getName(), null);
        plugin.configure(model, new ListBuilder<String>().add(
          emptyQuestionMappingFile.toAbsolutePath().toString()).toList());
        var result = plugin.processRecord(step);

        System.out.println("Should write? " + result.shouldWrite());
        System.out.println("Old value: " + new JSONObject(
          result.getRow().getOrigParamFiltersString()).toString(2));
        System.out.println(
          "New value: " + result.getRow().getParamFilters().toString(2));
      }
    }
  }
