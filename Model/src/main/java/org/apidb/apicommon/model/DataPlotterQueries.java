...
public class DataPlotterQueries {
  ...
  private static final String RNG_FILE_LOCATION = "/lib/rng/dataPlotterQueries.rng";
  private static final String XML_FILE_LOCATION = "/lib/xml/dataPlotter";
  ...
  public static Map<String,String> getQueryMap(String projectId, Category category) {
    if (_queries == null) {
      synchronized(DataPlotterQueries.class) {
        if (_queries == null) {
  ...
  /**
   * @param projectId project ID of queries to include
   * @return all DataPlotter query maps by category
   */
  ...
  private static Map<Category,List<WdkModelText>> loadQueries() {
    try {
      ...
      throw new WdkRuntimeException("Unable to load DataPlotter queries", e);
    }
  }
  ...
  private static Digester configureDigester() {
    ...
    digester.addObjectCreate("dataPlotter", ArrayList.class);
    configureNode(digester, "dataPlotter/query", WdkModelText.class, "add");
    digester.addCallMethod("dataPlotter/query", "setText", 0);
    return digester;
  }

  public static void main(String[] args) {
    if (args.length != 1) {
      System.err.println("USAGE: fgpJava " + DataPlotterQueries.class.getName() + " <projectId>");
      System.exit(1);
    }
    for (Category category : Category.values()) {
