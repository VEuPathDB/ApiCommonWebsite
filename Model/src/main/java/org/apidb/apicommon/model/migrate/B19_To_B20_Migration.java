...
public class B19_To_B20_Migration {
  ...
  private static DatabaseInstance parseArgs(final String[] args) {
    if (args.length == 3 || args.length == 4 && args[0].equals("-test")) {
      ...
      TEST = (args.length == 4);
      return new DatabaseInstance(SimpleDbConfig.create(SupportedPlatform.ORACLE, args[0 + argOffset],
          args[1 + argOffset], args[2 + argOffset], (short) 3));
    }
  ...
  private static void migrateGBrowseIds(DataSource ds) {
    ...
    if (TEST) {
      ...
      new SQLRunner(ds, UPDATE_GB_USERNAME_SQL).executeUpdateBatch(updateParams);
      System.out.println("Deleting out-of-date user mappings...");
      new SQLRunner(ds, DELETE_OUTDATED_MAPPINGS_SQL).executeUpdateBatch(userIdsToRemove);
      System.out.println("Deleting out-of-date users...");
      new SQLRunner(ds, DELETE_OUTDATED_USERS_SQL).executeUpdateBatch(userIdsToRemove);
      System.out.println("Deleting out-of-date sessions...");
      new SQLRunner(ds, DELETE_OUTDATED_SESSIONS_SQL).executeUpdateBatch(sessionIdsToRemove);
    }
  }
}
