...
public class SpanFromQuestionStageHandler extends ShowSpanStageHandler {
  ...
  public StepBean getChildStep(ActionServlet servlet, HttpServletRequest request,
    ...
    if (questionName == null || questionName.length() == 0)
      throw new WdkUserException("Required " + CConstants.QUESTION_FULLNAME_PARAM + " is missing.");
    ...
    if (hasWeight) {
      if (!strWeight.matches("[\\-\\+]?\\d+"))
        throw new WdkUserException("Invalid weight value: '" + strWeight + "'. Only integers are allowed.");
    ...
    StepBean childStep = user.createStep(strategyId, question, params, filterName, false, true,
        weight);
