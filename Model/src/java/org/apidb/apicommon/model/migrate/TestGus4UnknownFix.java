...
public class TestGus4UnknownFix {
  ...
  private static final String[] DISPLAY_PARAM_VALUES = {
  ...
  public void testFilterFix() throws Exception {
    ...
    try {
      ...
      for (int i = 0; i < QUESTIONS.length; i++) {
        ...
        Question question = model.getQuestion(step.getQuestionName());
        boolean modified = Gus4StepTableMigrator.fixFilterParamValues(result, question);
        System.out.println("Return value: " + modified);
        System.out.println("Object value: " + result.isModified());
  ...
  private StepData getStep(int i) {
    ...
    step.setOrigParamFiltersString(DISPLAY_PARAM_VALUES[i]);
    step.setParamFilters(new JSONObject(DISPLAY_PARAM_VALUES[i]));
    step.setQuestionName(QUESTIONS[i]);
    return step;
  }
}
