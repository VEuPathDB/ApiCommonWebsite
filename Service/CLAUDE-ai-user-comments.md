...
- `upstream-unavailable { reason }` — NCBI was down/unreachable; never cached.
...
`text-unavailable` and `upstream-unavailable` split what used to be a single outcome, because they call for opposite user actions. `text-unavailable` means NCBI answered and this paper has no usable open-access full text — retrying is pointless, uploading the PDF is the way forward. `upstream-unavailable` means NCBI itself could not serve us, so we learned *nothing* about the paper — the same submission may well succeed later, and a *different* publication would fail identically. The FE messaging must not tell an outage victim to try a different article. (The upload path never calls NCBI, so "upload a PDF instead" remains a working escape hatch during a total outage — it is the promoted action in that state.)
...
- `queued` → `fetching-article` → `scanning-gene-mentions` → `generating-summary` → `generating-pds` → `persisting`

`persisting` (writing the `comment_ai_run` cache row) now always runs for a cacheable outcome — it is no longer gated on `create_user_comment`. Gene-synonym resolution happens in the sync prelude before any `running` state is published, so it isn't an emitted stage. `generating-pds` (the compulsory product-description stage) runs only on the success path — after `generating-summary`, before `persisting` — so it is never emitted for the `gene-not-mentioned` / `mentioned-in-passing` short-circuits (no summary to derive from).
...
| ③ | `generating-summary` | Anthropic API call. System prompt + user prompts loaded from `resources/ai/prompts/getGeneSummary/{system.txt,user.txt,schema.json}`. Placeholder substitution: `[PAPER_TEXT]`, `[GENE]`, `[N_QUOTES]`. Schema enforced via structured outputs (not `[JSON_SCHEMA]` in-prompt). Strip markdown fences from response, parse JSON. On the response's `only_in_passing=true` → short-circuit to terminal `mentioned-in-passing` (persisted to `comment_ai_run`). Up to `max_retry=3` retries via formatter LLM on malformed JSON (port `extract_json` retry loop). |
| ③.5 | `generating-pds` | **Compulsory** product-description stage (port of Python `generatePDs`; the `verifyPDs`/`selectPD` passes are *not* ported). Runs only on the success path (guarded by `!isTerminal()`), so it never runs for the `gene-not-mentioned` / `mentioned-in-passing` short-circuits. Prompts from `resources/ai/prompts/generatePDs/{system.txt,user.txt,schema.json}`. Placeholders `[N_PDs]` (=3), `[GENE]` (reuses `geneForPrompt`), `[SUMMARY]` (the summary's `GeneSummary[].bullet_point`s joined by `\n`, via the `collectBullets` port). Response `{gene_id, short_summary, PDs:[{description, evidence_code, code_reason}]}`; we render only `PDs` (ignore `short_summary`). **Failure is fatal:** after the client's 3 formatter retries `complete()` throws → `run()` catch → terminal `internal-error`, nothing cached — so a PD-less "success" is never written to the cache. |
| ④ | flatten | Compute `ai_output.headline = Headline` (plain text). `ai_output.content` = bullet-flattened plain text that is also valid markdown — an "Executive summary:" section, "Details:" bullets, the **"AI-suggested product description(s):" section** (heading pluralised by count; `- <description> [<evidence_code>]` + indented `code_reason`), then "Evidence:", "Additional inferences", and an "Aliases mentioned in paper:" line. The PD section sits **between Details and Evidence**. No HTML; we stay plain-text-compatible while future-proofing for a markdown renderer on the show page (deferred). |
...
> **Validation stage removed (2026-06-05):** an earlier draft had a stage ④ `validating` (an Anthropic call using a `verifyGeneSummary` prompt) gated on `options.validate`. The Python authors reported the second pass didn't materially improve results and wasn't worth the tokens, so it's dropped — there is no `validate` option, no `validating` stage, and no `validation-error` terminal.

> **Product descriptions made compulsory (2026-07-01):** the `generating-pds` stage (Python `generatePDs`, single LLM call) is now a permanent, unconditional part of the pipeline — it derives its PDs from the first-pass `getGeneSummary` output. The pipeline therefore runs **two** LLM stages on the success path (`generating-summary` → `generating-pds`). The `generate_product_description` option was removed (it now gates nothing) and `PROMPT_VERSION` bumped `"1" → "2"` to invalidate every run cached during testing. Only `generatePDs` is ported — the Python `verifyPDs`/`selectPD` passes are intentionally omitted (same tokens-vs-benefit reasoning as the dropped validation pass).
...
**Terminal outcomes that are persisted to `comment_ai_run`** (and therefore short-circuit future submits, and are publishable): `success`, `mentioned-in-passing`, `gene-not-mentioned`. **Not persisted** (retries are free; not publishable): `text-unavailable`, `upstream-unavailable`, `internal-error`, `cancelled`. See "Out of scope / coordination items" for an optional `text-unavailable` observability log.
...
4. **Product descriptions** (Python `generatePDs`): ~~deferred~~ **DONE (2026-07-01)** — now a compulsory `generating-pds` stage rendering an "AI-suggested product description(s):" section into the flattened `ai_content` (so it rides in the existing `comment_ai_run.ai_content`, no new persistence target). Not gated by any option (`generate_product_description` removed); `PROMPT_VERSION` bumped to `"2"`. PDs derive from the **first-pass `getGeneSummary` output**. Only `generatePDs` was ported; the Python `verifyPDs`/`selectPD` passes were intentionally omitted.
...
9b. **Upstream unavailable** (D9): with the BioC host unreachable (e.g. point `BIOC_URL_BASE` at a closed port, or repeat during a real NCBI outage), POST any PMID → terminal `upstream-unavailable`, **no rows** written, and a `LOG.error` line carrying the status and body snippet. The FE must show the "try again later / NCBI" message with **Upload a PDF instead** first — and uploading a PDF for the same gene must still succeed end-to-end, since that path never calls NCBI.
...
| 10 | **Compulsory `generating-pds` stage** (product descriptions) | ✅ **Done (2026-07-01, TDD)** — `collectBullets` + `flattenProductDescriptions` (pure) + `generateProductDescriptions` stage + `run()` wiring; `PROMPT_VERSION "1"→"2"`; `generate_product_description` option removed; new `generatePDs` prompt resources; cosmetic prompt typos fixed. Full Service build green, **99 ai-package tests pass**. Real Anthropic `generatePDs` call **NOT live-tested** (needs `CLAUDE_API_KEY`); only parse/render/wiring unit-tested via injected client. |
...
- **`PROMPT_VERSION = "2"`** (`AiSummaryConfig`) — manually bumped when any prompt file changes; baked into digest. Covers both LLM stages (`getGeneSummary` + the compulsory `generatePDs`). Bumped `"1" → "2"` on 2026-07-01 when PDs became compulsory (new stage + cosmetic prompt typo fixes) to invalidate every run cached during testing. The `generate_product_description` option was removed at the same time, so the canonical options JSON is now `{}`.
...
**Implemented + unit-tested (pure):** `SyncPrelude.validate` (7 tests), `JobDigest` (8 tests), `JobRegistry.submit/attach/get` (6 tests), `PmcBiocFetcher.parseBiocJson` (7 tests, D2) + `PmcBiocFetcher.fetch` failure taxonomy (12 tests, D9), `AiGenePublicationPipeline.fetchArticle` upload/pubmed/text-unavailable (3 tests, D2) + upstream-unavailable terminal & wire shape (2 tests, D9), `GeneMentionScanner.countSubstrings`/`namesMentioned` (20 tests, D3), `AiGenePublicationPipeline.scanGeneMentions` gene-found/gene-not-mentioned (2 tests, D3), `PromptLoader` (6 tests, D4), `AnthropicJsonClient.extractJson`/retry loop (8 tests, D4), `AiGenePublicationPipeline.geneForPrompt`/`generateSummary` (6 tests, D4).
...
- **D9 (NCBI outage handling + fetcher logging): ✅ DONE.** Prompted by a real incident: on **2026-08-10** the whole `https://www.ncbi.nlm.nih.gov/research/bionlp/RESTful/*` tree returned `503 no healthy upstream` (its istio gateway had no healthy backend) for hours, for every PMID and every format, while the rest of NCBI — including the BioC docs page and the PubTator3 API — was fine. Two defects surfaced: (a) we reported it to users as `text-unavailable`, i.e. "this article has no full text", sending them to try other articles that could not possibly work; (b) `PmcBiocFetcher` had **no logging at all**, so the server logs looked completely normal throughout.
  - **Failure taxonomy split by owner.** `IOException` (connect failure / DNS / read timeout), HTTP **5xx**, and HTTP **429** → new `PmcBiocFetcher.UpstreamUnavailableException` → terminal `upstream-unavailable`. Everything else — other non-2xx, non-`application/json` content type, malformed JSON, empty parse — stays `TextUnavailableException` → `text-unavailable`. (The non-JSON branch is how this endpoint reports a non-OA paper, so it is correctly a paper-level problem.) New `JobStatus.UPSTREAM_UNAVAILABLE("upstream-unavailable")` — terminal, **not** publishable, **not** persisted, so `persist()`'s existing three-terminal whitelist already excludes it; `TerminalResult.upstreamUnavailable(reason)` renders `{type, reason}`.
  - **Logging** (log4j, house style): `LOG.error` for upstream failures with PMID, full URL, status, elapsed ms and the **first 200 chars of the body** — during this incident that snippet reads `no healthy upstream`, which is the single string that identified it as NCBI's problem; `LOG.warn` for paper-level failures naming the check that tripped; `LOG.info` on success with elapsed ms and extracted char count.
  - **Transport seam replaced.** The old package-private `PmcBiocFetcher(HttpClient)` seam was unused and impractical to fake (abstract class + 8-method `HttpResponse`). Replaced with a narrow `BiocHttpExchange` functional interface returning a `BiocResponse(statusCode, contentType, body)` record — same pattern as `LlmCompleter` / `AiRunStore`. Every branch of the taxonomy is now unit-testable from a lambda with no network. **+12 `PmcBiocFetcherTest` tests** (7→19) and **+2 pipeline tests** (21→23); **159 ai-package tests pass**.
  - **FE (web-monorepo, branch `ai-comments-ncbi-error-handling`).** Added `{ type: 'upstream-unavailable'; reason: string }` to `AiGenePublicationJobStatus`, a decoder case in `UserCommentsService.deserializeJobStatus`, the status to `TerminalErrorStatus` and both controller switches, and a distinct `AddView` message that names NCBI as the likely cause, advises trying again later, and links the support page. `TerminalRecoveryButtons` gained a `variant` prop: in the outage state **"Upload a PDF instead" is promoted to first** (that path never calls NCBI, so it still works) and the retry button reads **"Try again"** rather than "Try a different publication" — which would be actively wrong advice during a total outage. `restartForm` already preserves the typed PMID, so "Try again" returns to a pre-filled form. `tsc --noEmit` clean; genomics-site has no test infrastructure, so no FE tests were added.
  - **⚠ Deploy note:** `deserializeJobStatus` **throws** on an unrecognised `type`, so the FE and BE must ship together (confirmed with the user — no tolerant-decoder fallback was added).
  - **Deliberately not done:** automatic retry/backoff on 5xx. The observed outage was total and multi-hour, so in-request retries would only tie up request threads; recovery is user-driven by design.
  - **Alternative endpoint recorded in code** (javadoc on `BIOC_URL_BASE`, comment only — no code path): PubTator3, `…/research/pubtator3-api/publications/export/biocjson?pmids={pmid}&full=true`, served this same article normally throughout the outage. Its envelope differs — `{"PubTator3":[{passages:[…]}]}` with passages directly on the doc rather than under `documents` — so `parseBiocJson` would need to accept both shapes; the `section_type` vocabulary is identical. Worth revisiting if these outages recur.
...
### A. Upstream Python bugs / typos — ☐ NOT yet reported to the Python team
- ☐ **`DISCUSSION` vs `DISCUSS`** (D2, `PmcBiocFetcher`): Python `PUBMED_SECTIONS` keeps `DISCUSSION`, but the live PMC BioC vocabulary emits `DISCUSS`, so Python silently drops *all* discussion passages. Java uses `DISCUSS` (verified vs PMID 17299597). This is a *functional* difference (it changes which text reaches the model), so it stays and should still be reported upstream. Prompts were tuned on output that excluded discussion text → re-tuning may be warranted.
- ☑ **Cosmetic prompt typos — FIXED in Java (2026-07-01), not reproduced.** The earlier convention reproduced typos verbatim because the prompts were tuned against that exact output; that rationale collapsed once we moved to Sonnet 4.6 (off the tuned model) and bumped `PROMPT_VERSION` to `"2"` (nothing cached to protect). Fixed: `getGeneSummary/system.txt` `"The geneor one of"` → `"The gene or one of"` and `"measurements   - Be"` → newline-separated bullets; the formatter-prompt `"respong"` → `"Respond"`; and the newly-ported `generatePDs/system.txt` (`"experimetns"`, `"ortologue"`, `"except when your know"`, `"removed asdescribed"`, `"DO NOTstrip"`, `"Brainstorming-"` missing newline). Guarded by `PromptLoaderTest`. **Report to the Python team as fixes** (not just bug reports). (The `"7. FORMAT COMPLIANCEStructure"` typo was already gone — that section was rewritten for structured outputs.)

### B. Deliberate Java deviations (intentional — not bugs to report)
- **Only `generatePDs` ported; `verifyPDs`/`selectPD` omitted** (D10, 2026-07-01): the Python PD flow is three LLM calls (brainstorm → QC-against-paper → pick-one). The Java port runs only the first (brainstorm) and renders its full suggestion list as "AI-suggested product description(s):", matching the same tokens-vs-benefit reasoning that dropped the `verifyGeneSummary` pass. `short_summary` from the `generatePDs` response is parsed but not rendered (the FE already shows the summary headline).
