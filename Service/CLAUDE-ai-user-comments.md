...
| 7b | **`sibling_summary` aggregate** — counts over `comment_ai_provenance` (`reviewed`/`edited`/`latest_at`) wired into the cache-hit + live terminal responses | ⬜ Next — split out of D7; meaningful only now that publish writes provenance rows |
...
**Still stubs (throw `UnsupportedOperationException` / 501):** DELETE endpoint (501). _(`validateSummary` was removed, not stubbed — see the 2026-06-05 plan change. `persist`/`InsertCommentAiRunQuery.getArguments` implemented in D6; `InsertCommentAiProvenanceQuery.getArguments` + the publish endpoint implemented in D7.)_
