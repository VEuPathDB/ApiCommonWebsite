...
| create | `ApiCommonWebsite/Service/src/main/java/org/apidb/apicommon/service/services/ai/article/PdfTextExtractor.java` (PDFBox wrapper around a `Path` resolved via `TemporaryFileService`) |
...
| modify | `ApiCommonWebsite/Service/pom.xml` — add Apache PDFBox dependency (e.g. `org.apache.pdfbox:pdfbox:3.0.x`). `anthropic-java` is already present in the Model pom; no SDK addition needed there. |
